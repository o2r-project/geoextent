...
def test_geojson_extract_bbox():
    result = geoextent.fromFile('testdata/geojson/muenster_ring_zeit.geojson', bbox=True)
...
def test_invalid_coordinate_geojson_extract_bbox():
    with pytest.raises(Exception) as excinfo:
        geoextent.fromFile('testdata/geojson/invalid_coordinate.geojson')
    assert "is not valid" in str(excinfo.value)

def test_one_point_geojson_extract_bbox():
    result = geoextent.fromFile('testdata/geojson/onePoint.geojson', bbox=True)
...
def test_empty_file_geojson_extract_bbox():
    with pytest.raises(Exception) as excinfo:
        geoextent.fromFile('testdata/geojson/empty.geojson',  bbox=True)
    assert "is not valid" in str(excinfo.value)

def test_geojson_extract_time():
    result = geoextent.fromFile('testdata/geojson/muenster_ring_zeit.geojson', tbox=True)
    assert result["tbox"] == ['2018-11-14', '2018-11-14']

def test_geojson_extract_only_time():
    result = geoextent.fromFile('testdata/geojson/muenster_ring_zeit.geojson', bbox=False, tbox=True)
